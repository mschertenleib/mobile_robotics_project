...
async def turn_right(node,client,motor_speed, rotation_time):
...
async def turn_left(node,client,motor_speed, rotation_time):
    # Set the motor speeds in opposite directions to induce rotation
    await node.set_variables(set_motor_speed(-motor_speed*2, motor_speed*2))  
    # Allow time for the rotation 
    await client.sleep(rotation_time)
...
async def avoid_obstacles(node,client,state,side,obst, HTobst, LTobst, motor_speed, step_back_time, obst_gain, rotation_time):     
    ...
    if state == 0:
        ...
        if case == 0 or case == 1:
            ...
            if case == 0:
                await turn_right(node,client,obst_gain * ((obst[0]+obst[1]) // 100), 0.3)
            elif case == 1:     
                await turn_left(node,client,obst_gain * ((obst[4]+obst[3]) // 100), 0.3)
            ...
            if side:
                ...
                await turn_left(node,client,motor_speed, rotation_time)
                
            elif not side:
                await step_back(node,client,motor_speed, step_back_time)
                await turn_right(node,client,motor_speed, rotation_time)